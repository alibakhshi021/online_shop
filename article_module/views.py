...
def article_categories_component(request: HttpRequest):
    article_main_categories = ArticleCategory.objects.filter(
    ...
    return render(
        request, "article_module/components/article_category_component.html", context
    )
    ...
    if request.user.is_authenticated:
        ...
        context = {
            "comments": ArticleComment.objects.filter(
                article_id=article_id, parent=None
            ).prefetch_related("articlecomment_set"),
            "comments_count": ArticleComment.objects.filter(
                article_id=article_id
            ).count(),
        }
        return render(
            request, "article_module/includes/article_commend_partial.html", context
        )
